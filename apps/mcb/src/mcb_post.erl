...
construct(BoardId, Opts) when is_binary(BoardId) ->
  ThreadId  = mcb:propget(thread_id, Opts),
  Author    = mcb:propget(author,    Opts),
  Email     = mcb:propget(email,     Opts),
  Subject   = mcb:propget(subject,   Opts),
  Message   = mcb:propget(message,   Opts),
  DeletePw  = mcb:propget(deletepw,  Opts),
  PosterId  = mcb:propget(poster_id, Opts),
  Ident     = mcb:propget(ident, Opts),
  ...
  case string:strip(binary_to_list(Message), both) of
    ...
    _ ->
      ...
      PostId = mcb:as_binary(mcb_board:next_post_id(BoardId)),
      P = #mcb_post{
        thread_id   = mcb:as_binary(ThreadId)
        , post_id     = PostId
        , board_id    = BoardId
...
get(BoardId, PostId) when is_binary(BoardId), is_binary(PostId) ->
  PKey = mcb_db:key_for(mcb_post, {BoardId, PostId}),
  case mcb_db_riak:read(mcb_post, PKey) of
    {ok, P = #mcb_post{}} -> {ok, P};
    {error, _} = E -> E
  end.
...
new(BoardId, Opts) when is_binary(BoardId) ->
  case ?MODULE:construct(BoardId, Opts) of
    {ok, Post0} ->
      Post  = mcb_post:write_attach_set_ids(Post0, Opts),
      ThreadId = mcb:propget(thread_id, Opts),
      mcb_thread:add_post(BoardId, ThreadId, Post),
      {ok, Post};
    {error, E} ->
      {error, E}
  end.
...
write_attach_set_ids(P = #mcb_post{}, Opts) ->
  ...
  AttachList = mcb:propget(attach, Opts),
  AttachIds0 = [mcb_attach:write(A) || A <- AttachList],
...
delete(BoardId, PostId) ->
  ...
  TDKey = mcb_db:key_for( mcb_thread_dynamic
  ...
  Upd = fun(TD = #mcb_thread_dynamic{ post_ids=L }) ->
            ...
            case L2 of
              [] -> mcb_board_worker:thread_delete(
                      BoardId, P#mcb_post.thread_id);
              _ -> ok
            end,
            mcb_thread:touch_thread_dynamic(
              TD#mcb_thread_dynamic{ post_ids=L2 }
             )
        end,
  case mcb_db_mnesia:update(mcb_thread_dynamic, TDKey, Upd) of
    {atomic, _} ->
...
delete_dirty(BoardId, PostId) ->
  ...
  case ?MODULE:get(BoardId, PostId) of
    ...
    {ok, P = #mcb_post{}} ->
      ...
      lists:foreach(fun(AttId) -> AttachMod:delete(AttId) end,
                    P#mcb_post.attach_ids),
      mcb_db_riak:delete(mcb_post, PKey),
