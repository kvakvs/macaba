...
-define(CONF_FILENAME,    "etc/macaba.config").
...
get_or_fatal(Key) ->
  case gen_server:call(?SERVER, {get, Key}) of
    {ok, V} -> {ok, V};
    {error, not_found} ->
      mcb:fatal("Config parameter not found", Key);
    {error, What} ->
      mcb:fatal("Config access error", {{parameter, Key}, {error, What}})
  end.
...
init([]) ->
  case load_config(#conf_state{}) of
    State=#conf_state{} ->
      ...
      {ok, State};
    {error, Err1} ->
      mcb:fatal("Loading config " ?CONF_FILENAME, Err1)
  end.
...
traverse([K|K2], Tree) ->
  case mcb:propget(K, Tree, undefined) of
