...
%%
%% op: The operation function call
%%
...
write(Client, StatName, Op, Val) ->
  ReqID = mk_reqid(),
  mcbd_write_fsm_sup:start_write_fsm(
    [ReqID, self(), Client, StatName, Op, Val]),
  {ok, ReqID}.
...
prepare(timeout, SD0=#state{client=Client,
  ...
  Preflist = riak_core_apl:get_apl(DocIdx, ?N, mcbd_vnode),
