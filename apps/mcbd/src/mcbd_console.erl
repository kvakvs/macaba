%% @doc Interface for macaba-admin commands.
-module(mcbd_console).
-export([join/1,
         leave/1,
         remove/1,
...
join([NodeStr]) ->
    try riak_core:join(NodeStr) of
        ok ->
            lager:info("Sent join request to ~s\n", [NodeStr]),
            ok;
        {error, not_reachable} ->
            lager:error("Node ~s is not reachable!\n", [NodeStr]),
            error;
        {error, different_ring_sizes} ->
            lager:error("Failed: ~s has a different ring_creation_size~n",
                      [NodeStr]),
            error
    catch
        Exception:Reason ->
...
remove_node(Node) when is_atom(Node) ->
    try catch(riak_core:remove_from_cluster(Node)) of
        {'EXIT', {badarg, [{erlang, hd, [[]]}|_]}} ->
            ...
            lager:error("Leave failed, this node is the only member.~n"),
            error;
        Res ->
            lager:info(" ~p\n", [Res])
    catch
...
ringready([]) ->
    try riak_core_status:ringready() of
        ...
        {error, {different_owners, N1, N2}} ->
            lager:error("FALSE Node ~p and ~p list different partition owners\n",
                        [N1, N2]),
            error;
        {error, {nodes_down, Down}} ->
            lager:error("FALSE ~p down.  All nodes need to be up to check.\n",
                        [Down]),
            error
    catch
        Exception:Reason ->
